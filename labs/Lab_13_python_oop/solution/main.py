...
if __name__ == "__main__":
    ...
    for payment in payment_service.find_all():
        ...
        quartal_payments.update(
            {quartal: [payment] + quartal_payments.get(quartal, [])}
        )
    quartal_clients = {}
    for quartal, payments in quartal_payments.items():
        ...
        for payment in payments:
            clients_data.update(
                {payment.client_id: 1 + clients_data.get(payment.client_id, 0)}
            )
        quartal_clients.update({quartal: clients_data})
    for quartal in quartal_clients.keys():
        quartal_clients[quartal] = list(
            dict(
                sorted(
                    quartal_clients[quartal].items(),
                    key=lambda item: item[1],
                    reverse=True,
                )[:10]
            ).keys()
        )
    quartal_clients = dict(
        sorted(quartal_clients.items(), key=lambda item: item[0], reverse=True)
    )
    ...
    clients_areas = dict(
        sorted(clients_areas.items(), key=lambda item: item[1], reverse=True)[:10]
    )

    area_stats_block = AreaStatsBlock(area_data={"Russia": clients_areas})

    balances = {}
    for payment in payment_service.find_all():
        balances.update(
            {payment.client_id: payment.amount + balances.get(payment.client_id, 0)}
        )
    profiteers = dict(
        sorted(balances.items(), key=lambda item: item[1], reverse=True)[:10]
    )
    debtors = dict(
        sorted(balances.items(), key=lambda item: item[1], reverse=False)[:10]
    )

    max_min_balance_block = MaxMinBalanceBlock(debtors=debtors, profiteers=profiteers)
    ...
    XlsxReportWriter(
        ...
        f"my_payments_analytics_{date.today().year}_{date.today().month}_{date.today().day}"
    )
