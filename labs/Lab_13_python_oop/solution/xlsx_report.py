...
class QuartalMostActiveClientsBlock(ReportDataBlock):
    ...
    def tables(self):
        ...
        for quartal, clients_ids in self.quartal_clients.items():
            table.update({str(quartal): clients_ids})
...
class AreaStatsBlock(ReportDataBlock):
    ...
    def tables(self):
        ...
        for country_name, country_data in self.area_data.items():
            table_view_country_data = {"Город": [], "Количество клиентов": []}
...
class MaxMinBalanceBlock(ReportDataBlock):
    ...
    def tables(self):
        table = {
            "Задолжности": {"Клиент": [], "Состояние счёта": []},
            "Прибыльность": {"Клиент": [], "Состояние счёта": []},
        }
...
class ReportMetaBlock(ReportDataBlock):
    ...
    def tables(self):
        return {
            "Дата выгрузки": {self.dump_date.strftime("%Y-%m-%d"): []},
            "Период, за который сделана выгрузка": {
...
class XlsxReportWriter:
    ...
    def create_file(self, filename):
        workbook = xlsxwriter.Workbook(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), f"{filename}.xlsx")
        )
        ...
        def print_column(label: str, column_content, start_coords: coords) -> coords:
            ...
            if isinstance(column_content, list):
                ...
                if not label == None:
                    up_padding = 1
                    worksheet.write(
                        start_coords.row, start_coords.col, label, column_name_format
                    )
                for i, data in enumerate(column_content):
                    worksheet.write(
                        start_coords.row + i + up_padding,
                        start_coords.col,
                        data,
                        data_format,
                    )
                ...
                for subcolumn_name, subcolumn_content in column_content.items():
                    taken = print_column(
                        subcolumn_name,
                        subcolumn_content,
                        coords(start_coords.row + up_padding, start_coords.col + col),
                    )
                    col += taken.col
